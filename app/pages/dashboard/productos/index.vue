...
definePageMeta({ layout: 'dashboard', title: 'Productos' })

const { data: items, refresh } = await useFetch('/api/products', { key: 'cms-prod-list' })
...
<template>
  <div class="list-page">
    <div class="list-header">
      ...
      <NuxtLink to="/dashboard/productos/create" class="btn-primary">+ Añadir nuevo</NuxtLink>
    </div>
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="cell-actions">
              <NuxtLink :to="`/dashboard/productos/${item.id}`" class="action-link">Editar</NuxtLink>
              <button class="action-delete" @click="remove(item.id)">Eliminar</button>
            </td>
          </tr>
          <tr v-if="!items?.length">
            <td colspan="6" class="cell-empty">No hay productos aún.</td>
          </tr>
        </tbody>
      </table>
    </div>
