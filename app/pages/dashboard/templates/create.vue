...
definePageMeta({ layout: 'dashboard', title: 'Nuevo template' })
...
async function save() {
  ...
  try {
    ...
    await $fetch('/api/templates', { method: 'POST', body: { ...rest, content: { ...content, template_for: templateFor } } })
...
<template>
  <div class="form-page">
    <h1 class="page-title">Nuevo template</h1>
    <form @submit.prevent="save" class="w-[calc(100vw-180px)] overflow-x-hidden">
      <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="grid grid-cols-2 gap-4">
          ...
          <div class="field">
            ...
            <small class="text-gray-400 mt-1">Debe empezar con <code>tpl-</code></small>
          </div>
        </div>
      ...
      <div class="form-actions">
        ...
        <button type="submit" class="btn-primary" :disabled="saving">{{ saving ? 'Guardando...' : 'Crear template' }}</button>
      </div>
    </form>
  </div>
</template>
