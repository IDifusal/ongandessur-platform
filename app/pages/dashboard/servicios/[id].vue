...
definePageMeta({ layout: 'dashboard', title: 'Editar servicio' })

const route = useRoute()
const id = route.params.id as string

const { data: item } = await useFetch(`/api/services/${id}`, { key: `cms-serv-${id}` })

const form = reactive({ title: '', slug: '', description: '', image: '', published_at: '' })
...
async function save() {
  ...
  saving.value = true
  try {
    const body: Record<string, unknown> = { title: form.title, slug: form.slug, description: form.description, image: form.image }
    if (form.published_at) body.published_at = form.published_at
    else body.published_at = null
    await $fetch(`/api/services/${id}`, { method: 'PUT', body })
    await navigateTo('/dashboard/servicios')
  } catch (e: any) {
  ...
  <div class="form-page">
    ...
    <form @submit.prevent="save" class="cms-form">
      ...
      <div class="field">
        ...
        <RichEditor v-model="form.description" placeholder="Describe el servicio..." />
      </div>
      <div class="field">
      ...
      <div class="form-actions">
        <NuxtLink to="/dashboard/servicios" class="btn-cancel">Cancelar</NuxtLink>
