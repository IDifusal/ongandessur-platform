...
definePageMeta({ layout: 'dashboard', title: 'Noticias' })

const { data: items, refresh } = await useFetch('/api/news', { key: 'cms-news-list' })
...
<template>
  <div class="list-page">
    <div class="list-header">
      ...
      <NuxtLink to="/dashboard/noticias/create" class="btn-primary">+ Añadir nueva</NuxtLink>
    </div>
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td class="cell-actions">
              <NuxtLink :to="`/dashboard/noticias/${item.id}`" class="action-link">Editar</NuxtLink>
              <button class="action-delete" @click="remove(item.id)">Eliminar</button>
            </td>
          </tr>
          <tr v-if="!items?.length">
            <td colspan="4" class="cell-empty">No hay noticias aún.</td>
          </tr>
        </tbody>
      </table>
    </div>
